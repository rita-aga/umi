...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  - Core memory: {} KB", custom_config.core_memory_bytes / 1024);
    println!("  - Working memory: {} MB", custom_config.working_memory_bytes / (1024 * 1024));
    println!("  - Working memory TTL: {} seconds", custom_config.working_memory_ttl.as_secs());
    println!("  - Recall limit: {}", custom_config.default_recall_limit);
    println!("  - Embedding batch size: {}", custom_config.embedding_batch_size);
    println!();

    // === Disable Embeddings (Graceful Degradation) ===
    println!("--- Configuration: Without Embeddings ---");

    let no_embedding_config = MemoryConfig::default()
        .without_embeddings();
    ...
    println!("  Recall found {} results (using text search)", results.len());
    println!();

    // === Disable Query Expansion ===
    println!("--- Configuration: Without Query Expansion ---");

    let no_expansion_config = MemoryConfig::default()
    ...
    println!("  - Core memory: {} KB", combined_config.core_memory_bytes / 1024);
    println!("  - Recall limit: {}", combined_config.default_recall_limit);
    println!("  - Embeddings: {}", if combined_config.generate_embeddings { "enabled" } else { "disabled" });
    println!("  - Query expansion: {}", if combined_config.query_expansion_enabled { "enabled" } else { "disabled" });
    println!();
    ...
    println!("  Actual results returned: {} (using RecallOptions.with_limit(5))", limited_results.len());
    println!();
