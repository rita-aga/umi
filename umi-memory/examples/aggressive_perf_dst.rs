...
use std::sync::Arc;
use umi_memory::dst::{DeterministicRng, FaultConfig, FaultInjector, FaultType, SimConfig, Simulation};
...
async fn test_extreme_fault_rate() {
    ...
    let result = sim.run(|env| async move {
        ...
        println!("  90% fault rate: {} successes, {} failures (rate={:.1}%)",
        ...
        println!("  State: {} accesses, {} core entities", accesses, core_count);
...
async fn test_cascading_faults() {
    ...
    let result = sim.run(|env| async move {
        ...
        for i in 0..50 {
            match memory.remember(&format!("Alice cascading {}", i)).await {
                Ok(result) => {
                    ...
                    let has_note = result.entities.iter().any(|e|
                        format!("{:?}", e.entity_type).contains("Note"));
                    if has_note && i < 5 {
        ...
        println!("  Cascading faults: {} successes, {} storage errors", successes, storage_errors);
        ...
        println!("  Final state: {} accesses after {} successes", accesses, successes);
        ...
        if accesses > max_expected {
            println!("  POTENTIAL BUG: {} accesses > {} expected for {} successes",
...
async fn test_rapid_operations_under_faults() {
    ...
    let result = sim.run(|env| async move {
        ...
        println!("  500 rapid ops: {} successes, {} failures", successes, failures);
...
async fn test_alternating_faults() {
    ...
    let result = sim.run(|env| async move {
        ...
        let mut memory_faulty = UnifiedMemory::new(
            llm.clone(), embedder.clone(), vector.clone(),
            storage_faulty, env.clock.clone(), config.clone()
        );

        let mut phase1_failures = 0;
        for i in 0..30 {
            if memory_faulty.remember(&format!("Faulty {}", i)).await.is_err() {
                phase1_failures += 1;
            }
            let _ = env.clock.advance_ms(10);
        }
        ...
        let mut memory_healthy = UnifiedMemory::new(
            llm.clone(), embedder.clone(), vector.clone(),
            storage_healthy, env.clock.clone(), config.clone()
        );

        let mut phase2_failures = 0;
        for i in 0..30 {
            if memory_healthy.remember(&format!("Healthy {}", i)).await.is_err() {
                phase2_failures += 1;
            }
            let _ = env.clock.advance_ms(10);
        }
        ...
        if phase2_failures > 0 {
            println!("  BUG: {} failures in healthy phase - lingering corruption?", phase2_failures);
        } else {
        ...
        let mut memory_partial = UnifiedMemory::new(
            llm, embedder, vector,
            storage_partial, env.clock.clone(), config
        );

        let mut phase3_successes = 0;
        for i in 0..30 {
            if memory_partial.remember(&format!("Partial {}", i)).await.is_ok() {
                phase3_successes += 1;
            }
            let _ = env.clock.advance_ms(10);
        }
        println!("  Phase 3 (50% faults): {} successes / 30", phase3_successes);
