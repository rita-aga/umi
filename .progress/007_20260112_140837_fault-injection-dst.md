...
|--------|---------------|--------|
| EntityExtractor | LLM timeout/failure → fallback entity | Can't verify extraction degradation |
| DualRetriever | Storage/LLM failure → degraded results | Can't verify retrieval fallback |
| EvolutionTracker | LLM failure → no evolution detected | Can't verify evolution skip |
...
**Expected Behavior**:
- LLM timeout → fallback entity created
- Rate limit → fallback entity created
- Invalid response → fallback entity parsed or fallback created
...
**Expected Behavior**:
- Vector search timeout → fallback to text search
- LLM timeout (query expansion) → use original query
- Storage read fail → return empty or error (graceful)
- Multiple faults → still complete (degraded)
...
**Expected Behavior**:
- LLM timeout → return empty evolutions
- Rate limit → return empty evolutions
- Invalid response → return empty or parsed evolutions
...
1. ✅ Add `dst_tests` module to `extraction/mod.rs` with 4 fault injection tests
2. 🔍 Run: `cargo test extraction::dst_tests` - **EXPECT FAILURES**
   - Document what breaks (panic? unwrap? missing error handling?)
   - Take notes on failure modes
3. 🐛 **Analyze & Fix Issues**:
...
### Step 3: DualRetriever Tests (DISCOVERY MODE)

1. ✅ Add `dst_tests` module to `retrieval/mod.rs` with 4 fault injection tests
2. 🔍 Run: `cargo test retrieval::dst_tests` - **EXPECT FAILURES**
   - Document failures
3. 🐛 **Analyze & Fix Issues**:
   - Vector search timeout handling
   - Query expansion failure fallback
...
**Expected findings**:
- Missing fallback to text search
- Query expansion not skippable
- Storage errors not handled gracefully

### Step 4: EvolutionTracker Tests (DISCOVERY MODE)

1. ✅ Add `dst_tests` module to `evolution/mod.rs` with 4 fault injection tests
2. 🔍 Run: `cargo test evolution::dst_tests` - **EXPECT FAILURES**
   - Document failures
3. 🐛 **Analyze & Fix Issues**:
   - LLM timeout handling
   - Empty evolution return on failure
...
**Expected findings**:
- Evolution detection not skippable on LLM failure
- Missing empty return path
- Possible panic on malformed LLM response
...
- [x] Test 1 (LLM timeout): ✓ **VERIFIED PASS** - Fault fires, creates fallback entity (type=Note, name="Note: ...", confidence=0.5)
- [x] Test 2 (Rate limit): ✓ **VERIFIED PASS** - Same fallback path as timeout
- [x] Test 3 (Invalid response): ✓ **VERIFIED PASS** - JSON parse fallback works
- [x] Test 4 (Probabilistic 50%): ✓ **VERIFIED PASS** - DETERMINISTIC! Seed 42 + 50% rate = 10/10 failures (reproducible)
- [x] Test 5 (Service unavailable): ✓ **VERIFIED PASS** - Fallback entity created
...
- [ ] Test 1 (Vector timeout): _Document finding here_
- [ ] Test 2 (LLM timeout): _Document finding here_
- [ ] Test 3 (Storage fail): _Document finding here_
- [ ] Test 4 (Multiple faults): _Document finding here_

### EvolutionTracker Discoveries
- [ ] Test 1 (LLM timeout): _Document finding here_
- [ ] Test 2 (Rate limit): _Document finding here_
- [ ] Test 3 (Invalid response): _Document finding here_
- [ ] Test 4 (Probabilistic): _Document finding here_

---

## Success Criteria

- [ ] EntityExtractor has 4+ fault injection tests
- [ ] DualRetriever has 4+ fault injection tests
- [ ] EvolutionTracker has 4+ fault injection tests
- [ ] All fault tests inject faults deterministically (same seed = same failures)
- [ ] All discovered bugs are fixed
- [ ] All tests pass after fixes
- [ ] Tests use Simulation::new().with_fault() pattern
- [ ] Tests validate specific graceful degradation behavior
- [ ] Findings documented in test comments and discovery log
- [ ] grep confirms FaultConfig/FaultType present in all three modules
