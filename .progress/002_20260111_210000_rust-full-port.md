...
### Files Created
- [x] `umi-core/src/llm/mod.rs` - Trait + errors + CompletionRequest
- [x] `umi-core/src/llm/sim.rs` - SimLLMProvider (wraps DST SimLLM)
- [x] `umi-core/src/llm/anthropic.rs` - AnthropicProvider (feature-gated)
- [x] `umi-core/src/llm/openai.rs` - OpenAIProvider (feature-gated)
- [x] `docs/adr/013-llm-provider-trait.md` - ADR for LLM Provider design
...
- 2026-01-11: **Phase 1 Complete** - LLM Provider Trait
  - Created `umi-core/src/llm/` module with 4 files
  - `LLMProvider` trait with `complete()` and `complete_json()`
  - `SimLLMProvider` wraps DST SimLLM (simulation-first!)
  - `AnthropicProvider` (feature: anthropic) - Claude API
  - `OpenAIProvider` (feature: openai) - OpenAI API + compatible APIs
  - `CompletionRequest` with builder pattern
  - `ProviderError` unified error type with `is_retryable()`
  - ADR-013 documents design decisions
  - 263 tests passing, 17 doc tests passing
  - Fixed SimLLM to use `Mutex` instead of `RefCell` for `Sync`
